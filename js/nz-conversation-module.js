...
(function () {
  ...
  /* ── Render: Level Selection (card grid, matches reference layout) ── */
      ...
      '<div class="nz-page nz-fadein">' +
        ...
        '<div class="nz-level-grid">' +
          LVLS.map(function (lvl) {
            var c = LEVEL_HEX[lvl];
            return '<button class="nz-level-card" data-level="' + lvl + '" style="border-color:' + c + '33;">' +
              '<div class="nz-level-bar" style="background:' + c + ';"></div>' +
              '<span class="nz-level-icon">' + LEVEL_ICON[lvl] + '</span>' +
              '<div class="nz-level-name" style="color:' + c + ';">' + lvl + '</div>' +
              '<div class="nz-level-meta">' + levelScenarioCount(lvl) + ' scenarios</div>' +
              '</button>';
          }).join('') +
        '</div>' +
      '</div>';
    root.querySelectorAll('.nz-level-card').forEach(function (btn) {
