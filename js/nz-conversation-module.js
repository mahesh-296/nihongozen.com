...
(function () {
  ...
  /* ── State ──────────────────────────────────────────────────── */
  /* NOTE: the "levels" step (an "All Levels" overview screen) has
     been removed — the Conversation section now always opens
     directly on the level selected in the sidebar's Level Filter,
     and re-renders automatically when that filter changes. */
  var state = {
  ...
  function renderList(root) {
    var list = SCENARIOS.filter(function (s) { return s.level === state.filter; });
  ...
  window.ConversationPage = {
    /* initialLevel: the level currently selected in the sidebar's
       Level Filter (window.NZ_LEVEL). The Conversation section no
       longer has an "All Levels" overview screen — it always opens
       directly on this level, and the caller re-mounts (see
       NZ_LEVEL_ROUTES in index.html) whenever the sidebar level
       changes so the list stays in sync. */
    mount: function (rootId, initialLevel) {
      var root = $id(rootId);
      if (!root) return;
      state.selected = null;
      state.isPlaying = false;
      state.filter = (initialLevel && LEVELS.indexOf(initialLevel) !== -1) ? initialLevel : 'N5';
      state.step = 'list';
      renderList(root);
    },
    cleanup: function () {
