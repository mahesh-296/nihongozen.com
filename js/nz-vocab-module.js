...
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px;">
  ...
  function renderFCContent() {
    ...
    if(front) front.innerHTML=`
      ...
      <p style="font-family:'JetBrains Mono',monospace;font-size:13px;color:var(--fg-muted);">${esc(word.romaji||'')}</p>
      <p style="font-size:11px;color:var(--fg-muted);margin-top:10px;">Click to reveal meaning</p>`;
    if(back) back.innerHTML=`
      <p style="font-size:22px;font-weight:700;color:var(--fg);text-align:center;margin-bottom:10px;">${esc(word.en)}</p>
      <p style="font-size:11px;color:var(--fg-muted);margin-top:6px;">Click to flip back</p>`;
    if(counter) counter.textContent=`${cardIndex+1} / ${filtered.length}`;
...
<div style="max-width:900px;margin:0 auto;padding:24px 16px;">
  <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">
    <button id="vocab-fc-back" aria-label="Back to Vocabulary" style="display:flex;align-items:center;justify-content:center;
      width:32px;height:32px;border-radius:9px;background:var(--card-elevated);border:1px solid var(--border);
      color:var(--fg-muted);cursor:pointer;font-size:15px;flex-shrink:0;">←</button>
    <h1 style="font-size:22px;font-weight:800;color:var(--fg);margin:0;letter-spacing:-0.3px;">🃏 Flashcards</h1>
  </div>
  <p id="vocab-count-label" style="font-size:13px;color:var(--fg-muted);margin:0 0 16px 42px;"></p>
