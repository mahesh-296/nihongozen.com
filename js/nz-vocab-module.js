...
<div style="max-width:1600px;margin:0 auto;padding:24px 16px;">
  ...
  <!-- ═══ Chapter Vocab by Level (below SRS, separate) ═══ -->
  <div id="nzcv-chapter-panel" style="margin-bottom:20px;">
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
      <span style="font-size:13px;font-weight:700;color:var(--fg-muted);">📖 課別語彙 Chapter Vocab by Level:</span>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:0;" id="nzcv-level-pills"></div>
    <div id="nzcv-chapter-body" style="display:none;margin-top:12px;
         background:var(--card-elevated,#1a1a1a);border:1px solid var(--border,#2a2a2a);
         border-radius:14px;overflow:hidden;"></div>
  </div>
  ...
  <div id="vocab-main-area"></div>
    ...
    /* Chapter Vocab by Level pills */
    (function initChapterPanel() {
      const CHAP_COLORS = { 5:'#22c55e', 4:'#06b6d4', 3:'#eab308', 2:'#a855f7', 1:'#ef4444' };
      const LEVEL_MAP   = { 5:'N5', 4:'N4', 3:'N3', 2:'N2', 1:'N1' };
      const pillsWrap   = document.getElementById('nzcv-level-pills');
      const body        = document.getElementById('nzcv-chapter-body');
      if (!pillsWrap || !body) return;

      let activeLvl = null;  // currently expanded level number (1–5)
      ...
      function renderChapterList(lvlNum) {
        const nLvl  = LEVEL_MAP[lvlNum];
        const col   = CHAP_COLORS[lvlNum];
        const data  = window.NZChapterData && window.NZChapterData[nLvl];
        if (!data || !data.length) {
          body.innerHTML = '<p style="padding:20px;color:var(--fg-muted);font-size:13px;">No data available.</p>';
          return;
        }
        // Render collapsible chapter list with word grid
        let openCh = null;
        function renderBody() {
          body.innerHTML = data.map((ch, ci) => {
            const isOpen = openCh === ci;
            const wordCount = (ch.words||[]).length;
            return `
<div style="border-bottom:1px solid var(--border,#2a2a2a);">
  <button data-ci="${ci}" style="width:100%;display:flex;align-items:center;justify-content:space-between;
    padding:12px 18px;background:transparent;border:none;cursor:pointer;font-family:inherit;
    color:var(--fg,#f0f0f0);text-align:left;transition:background 0.12s;"
    onmouseenter="this.style.background='rgba(255,255,255,0.04)'"
    onmouseleave="this.style.background='transparent'">
    <div style="display:flex;align-items:center;gap:10px;">
      <span style="padding:2px 8px;border-radius:6px;font-size:11px;font-weight:700;
                   background:${col}22;color:${col};">第${ch.ch}課</span>
      <span style="font-size:13px;font-weight:700;">${esc(ch.title||'第'+ch.ch+'課')}</span>
      ${ch.topic?`<span style="font-size:11px;color:var(--fg-muted);">${esc(ch.topic)}</span>`:''}
    </div>
    <div style="display:flex;align-items:center;gap:8px;">
      <span style="font-size:11px;color:var(--fg-muted);">${wordCount} words</span>
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
        style="transform:${isOpen?'rotate(180deg)':'rotate(0deg)'};transition:transform 0.2s;flex-shrink:0;">
        <path d="m6 9 6 6 6-6"/>
      </svg>
    </div>
  </button>
  ${isOpen ? `<div style="padding:12px 18px 20px;">
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:8px;">
      ${(ch.words||[]).map(w=>`
        <div style="border-radius:10px;border:1px solid var(--border,#2a2a2a);
             background:var(--card,#141414);padding:12px;border-left:3px solid ${col};
             cursor:pointer;transition:transform 0.15s,box-shadow 0.15s;"
             onmouseenter="this.style.transform='translateY(-2px)';this.style.boxShadow='0 6px 20px rgba(0,0,0,0.4)'"
             onmouseleave="this.style.transform='';this.style.boxShadow=''"
             onclick="(function(){if(!window.speechSynthesis)return;window.speechSynthesis.cancel();var u=new SpeechSynthesisUtterance('${esc(w.jp).replace(/'/g,"\\'")}');u.lang='ja-JP';u.rate=0.85;window.speechSynthesis.speak(u);})()" >
          <div style="font-family:'Noto Sans JP',sans-serif;font-size:18px;font-weight:700;
               color:var(--fg);margin-bottom:3px;">${esc(w.jp)}</div>
          <p style="font-family:'JetBrains Mono',monospace;font-size:10px;
               color:var(--fg-muted);font-style:italic;margin:0 0 4px;">${esc(w.romaji||'')}</p>
          ...
          // Re-attach chapter toggle handlers
          body.querySelectorAll('button[data-ci]').forEach(btn => {
            btn.addEventListener('click', () => {
              const ci = parseInt(btn.dataset.ci);
              openCh = (openCh === ci) ? null : ci;
              renderBody();
              if (openCh !== null) {
      ...
      function renderPills() {
        pillsWrap.innerHTML = [5,4,3,2,1].map(n => {
          const col   = CHAP_COLORS[n];
          const nLvl  = LEVEL_MAP[n];
          const isActive = activeLvl === n;
          return `<button data-lvlnum="${n}"
            style="padding:6px 18px;border-radius:8px;font-size:13px;font-weight:700;
                   background:${isActive ? col : 'transparent'};
                   border:2px solid ${col};color:${isActive ? '#fff' : col};
                   cursor:pointer;font-family:inherit;transition:all 0.15s;">
            Level ${n} <span style="font-size:11px;font-weight:500;opacity:0.8;">(${nLvl})</span>
          </button>`;
        }).join('');

        pillsWrap.querySelectorAll('button[data-lvlnum]').forEach(btn => {
          btn.addEventListener('click', () => {
            const n = parseInt(btn.dataset.lvlnum);
            if (activeLvl === n) {
              activeLvl = null;
...
// ============================================================
'use strict';
// NihongoZen — Chapter Controller (vocab deduplication only)
