...
var VocabPage = (() => {
  ...
  function getLevelChapters() {
    ...
    /* N2 uses the restructured Week-based navigation (Main Page → Week
       Page → Lesson) and always reads the full, unfiltered Sou Matome
       N2 chapter list so indices line up with the week/day grouping
       below — book-tab splitting doesn't apply to this flow. */
    if (activeLevel === 'N2') return all;
    const books = getBooksForLevel(activeLevel);
...
<div style="display:flex;flex-direction:column;align-items:center;max-width:380px;margin:0 auto;">
  ...
  /* All 56 raw N2 chapter entries (8 weeks × 7 days), unfiltered —
     this is the single source of truth the Main Page, Week Page and
     the existing List/Detail screens all index into via activeChapter. */
  function n2AllChapters() {
    return (typeof NZChapterData !== 'undefined' && NZChapterData.N2) || [];
  }
  ...
  function renderN2MainScreen(area) {
    ...
    const lbl = document.getElementById('vocab-count-label');
    if (lbl) lbl.textContent = `Nihongo Sou Matome N2 · 8 weeks`;
    ...
    area.innerHTML = `
      ...
      ${!weeks.length
        ? `<p style="color:var(--fg-muted);text-align:center;padding:40px;">No N2 chapters found.</p>`
