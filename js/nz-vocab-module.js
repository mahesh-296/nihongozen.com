...
/* ─────────────────────────────────────────────────────────────────────────────
   NOTE: The chapter-wise vocabulary feature (chapters.js / vocabLevel.js data
   files and the "📖 Chapters" button) has been removed from the app. Only the
   "📚 Basic Vocab" module remains. The data-bootstrap loader that used to fetch
   those two files has been removed along with them.
   ───────────────────────────────────────────────────────────────────────────── */
...
// The "📖 Chapters" button has been removed from the app — only the
// "📚 Basic Vocab" button remains. injectButton() is intentionally not
// auto-invoked anymore (it stays available on window.NZChapterVocab in
// case it's ever needed again, but nothing calls it).
...
(function injectVocabStyles() {
  ...
  style.textContent = `
    ...
    #bv-open-btn:hover { opacity:0.94; transform:translateY(-2px); box-shadow:var(--shadow-primary-hover); }
    #bv-open-btn:active { transform:translateY(0); }
...
<div style="display:flex;flex-direction:column;align-items:center;max-width:380px;margin:0 auto;">
  ...
  /* ── STEP 2: Chapter Cards (this is the "JLPT Level page") ──
     ...
     Chapter Search Bar → Chapters grid. */
  ...
  function renderListScreen(area) {
    ...
    `;
    document.getElementById('vocab-back-chapters')?.addEventListener('click', () => { stopPlayAll(); goToChapters(); });
...
// The "📖 Chapters" button has been removed from the app — only the
// "📚 Basic Vocab" button remains, so this is intentionally not auto-invoked.
