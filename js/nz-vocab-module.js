...
const NZChapterVocab = (() => {
  ...
  (function injectStyles() {
    ...
    s.textContent = `
      ...
      /* Vocab word card — JLPT level color theme.
         --jlpt-c is set inline per card to that level's color;
         used for the left accent bar plus the full-border highlight
         on hover / selection. */
      .nzcv-word-card {
        border-radius:12px; border:1.5px solid var(--border,#2a2a2a);
        border-left:4px solid var(--jlpt-c, var(--primary,#e8446a));
        background:var(--card,#141414);
        padding:16px; cursor:pointer; position:relative;
        transition:transform 0.18s, box-shadow 0.18s, border-color 0.18s;
      }
      .nzcv-word-card:hover,
      .nzcv-word-card.is-selected {
        transform:translateY(-3px);
        border-color:var(--jlpt-c, var(--primary,#e8446a));
        box-shadow:0 0 0 1px var(--jlpt-c, var(--primary,#e8446a)), 0 8px 28px rgba(0,0,0,0.4);
      }
      /* Level colors */
      .nzcv-lvl-N5  { color:var(--success); }
      .nzcv-lvl-N4  { color:var(--n4); }
      .nzcv-lvl-N3  { color:var(--n3); }
      .nzcv-lvl-N2  { color:var(--n2); }
      .nzcv-lvl-N1  { color:var(--n1); }
      /* Scrollbar */
  ...
  /* ── Colours per level ───────────────────────────────── */
  const COLORS = { N5:'var(--n5)', N4:'var(--n4)', N3:'var(--n3)', N2:'var(--n2)', N1:'var(--n1)' };
  ...
  function renderGrid() {
    ...
    area.querySelectorAll('.nzcv-word-card').forEach(card => {
      card.addEventListener('click', e => {
        ...
        area.querySelectorAll('.nzcv-word-card.is-selected').forEach(c => c.classList.remove('is-selected'));
...
/* ── Chapter button removed per UI/UX update ──────────────────────
   The "📖 Chapters" button that used to sit next to "Basic Vocab"
   has been removed from the header entirely. NZChapterVocab.open()
   itself, and window.NZChapterData (js/nz-chapter-data-full.js), are
   left completely intact — other features (chapter word lookups,
   related-word logic, etc.) still depend on that data. We simply no
   longer auto-inject the button that opened this overlay. */
...
(function injectVocabStyles() {
  ...
  style.textContent = `
    ...
    /* ── Card hover — JLPT level color theme ─────────────
       --jlpt-c is set inline per card; used for the accent bar
       (border-left) plus the full-border highlight on hover/select. */
    .vocab-card, .bv-card { border:1.5px solid var(--border); }
    .vocab-card:hover, .bv-card:hover,
    .vocab-card.is-selected, .bv-card.is-selected {
      transform:translateY(-3px);
      border-color:var(--jlpt-c, var(--primary,#e8446a));
      box-shadow:0 0 0 1px var(--jlpt-c, var(--primary,#e8446a)), 0 8px 32px rgba(0,0,0,0.35);
    }
    ...
    .srs-pill-N4 { color:var(--n4); border-color:var(--n4); }
    .srs-pill-N3 { color:var(--n3); border-color:var(--n3); }
    .srs-pill-N2 { color:var(--n2); border-color:var(--n2); }
    .srs-pill-N1 { color:var(--n1); border-color:var(--n1); }
    ...
    /* List step */
    /* Part 1 — Book Selection Bar (Tab Switcher) */
    .nz-book-tabs { display:flex; align-items:center; gap:8px; flex-wrap:wrap;
      margin-bottom:16px; }
    .nz-book-tab {
      padding:8px 16px; border-radius:var(--radius); font-size:13px; font-weight:700;
      font-family:var(--font-sans); cursor:pointer;
      background:var(--card-elevated); border:1px solid var(--border); color:var(--fg-muted);
      transition:background var(--duration) var(--ease), color var(--duration) var(--ease),
                 border-color var(--duration) var(--ease), transform var(--duration) var(--ease);
    }
    .nz-book-tab:hover { border-color:var(--nz-book-color, var(--primary)); color:var(--fg); }
    .nz-book-tab.active {
      background:var(--nz-book-color, var(--primary)); border-color:var(--nz-book-color, var(--primary));
      color:#fff;
    }
    @media (max-width:560px){ .nz-book-tab { flex:1 1 calc(50% - 8px); text-align:center; } }
    ...
    /* ══════════════════════════════════════════════════════
       VOCAB WORD CARD — same design system as the rest of the
       app. Everything below is built from tokens.css variables
       (--card, --border, --fg, --primary, --accent, --n5…) so it
       automatically matches Dark / Light Pink / Light Orange —
       no hardcoded colors, consistent with every other card in
       NihongoZen. Used for the per-word card in the vocab list
       AND the detail page, so both match exactly.
       ══════════════════════════════════════════════════════ */
    .nz-vcard-page { padding:0; }
    .nz-vcard-list { display:flex; flex-direction:column; gap:14px; }
    .nz-vcard {
    ...
    .nz-vcard:hover { transform:translateY(-3px); box-shadow:var(--shadow-lg); border-color:var(--border-strong); }
    .nz-vcard.nz-vcard-playing { border-color:var(--primary); box-shadow:var(--shadow-glow); }
    /* Head is a row only for the speak button; the word content itself
       (.nz-vcard-body) is always a vertical stack: Japanese → English → Romaji.
       English never sits beside Japanese. */
    .nz-vcard-head { display:flex; align-items:flex-start; gap:14px; }
    .nz-vcard-body { flex:1; min-width:0; display:flex; flex-direction:column; gap:4px; }
    .nz-vcard-speak {
    ...
    .nz-vcard-speak:hover { border-color:var(--primary); color:var(--primary); }
    .nz-vcard-speak.nz-vcard-speak-active { background:var(--primary); border-color:var(--primary); color:#fff; }
    .nz-vcard-num { font-family:var(--font-mono); font-size:13px; font-weight:700;
      color:var(--accent); margin-right:10px; }
    .nz-vcard-jp { font-family:var(--font-jp-sans); font-size:28px; font-weight:800;
      color:var(--fg); display:inline; word-break:break-word; }
    .nz-vcard-kana { font-family:var(--font-jp-sans); font-size:16px; color:var(--fg-muted);
      margin-left:10px; }
    .nz-vcard-romaji { font-family:var(--font-mono); font-size:13px; color:var(--fg-muted);
      font-style:italic; }
    .nz-vcard-en { font-family:var(--font-sans);
      font-size:16px; color:var(--fg); font-weight:600; line-height:1.4;
      overflow-wrap:anywhere; }
    .nz-vcard-sentence {
      margin-top:16px; background:var(--card-elevated); border-left:3px solid var(--primary);
      border-radius:var(--radius); padding:14px 16px;
    }
    .nz-vcard-sentence-jp { font-family:var(--font-jp-sans); font-size:17px; color:var(--fg);
      line-height:1.6; margin-bottom:6px; }
    .nz-vcard-sentence-kana { font-family:var(--font-jp-sans); font-size:13px; color:var(--fg-muted);
      line-height:1.6; margin-bottom:6px; }
    .nz-vcard-sentence-romaji { font-family:var(--font-mono); font-size:13px; color:var(--fg-muted);
      line-height:1.6; margin-bottom:6px; }
    .nz-vcard-sentence-en { font-family:var(--font-sans); font-style:italic;
      font-size:14px; color:var(--fg-muted); line-height:1.5; }
    .nz-vcard-generated-tag { font-size:10px; color:var(--fg-subtle); margin-top:8px;
      font-style:italic; }

    /* Play All / Stop controls — reuse the app's universal .btn system */
    .nz-playall-bar { display:flex; align-items:center; gap:10px; margin-bottom:16px; }
    .nz-playall-btn, .nz-stopall-btn {
      display:inline-flex; align-items:center; gap:7px; padding:9px 16px;
      border-radius:var(--radius); font-size:13px; font-weight:700; font-family:var(--font-sans);
    ...
    .nz-playall-btn:hover:not(:disabled) { filter:brightness(1.08); transform:translateY(-1px); }
    .nz-playall-btn:disabled { opacity:0.45; cursor:not-allowed; }
    .nz-stopall-btn { background:var(--card-elevated); border-color:var(--border); color:var(--fg); }
    .nz-stopall-btn:hover:not(:disabled) { border-color:var(--primary); color:var(--primary); }
    .nz-stopall-btn:disabled { opacity:0.45; cursor:not-allowed; }

    /* Detail page reuses the same card look, larger */
    .nz-vcard-detail-wrap { max-width:640px; margin:0 auto; }
    .nz-vcard-detail-wrap .nz-vcard { cursor:default; padding:28px 30px; }
    .nz-vcard-detail-wrap .nz-vcard-jp { font-size:36px; }
    .nz-vcard-detail-wrap .nz-vcard-en { font-size:18px; }
    .nz-vcard-detail-nav { display:flex; align-items:center; justify-content:space-between;
      margin-top:20px; }

    @media (max-width:560px){
      .nz-vcard-detail-wrap .nz-vcard-jp { font-size:28px; }
    }
  `;
...
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(210px,1fr));gap:12px;">
  ${filtered.map(word => `
    <div class="bv-card"
      data-id="${esc(word.id)}"
      style="--jlpt-c:${word.color};border-radius:12px;background:var(--card);
             padding:16px;cursor:pointer;border-left:4px solid ${word.color};
  ...
  `).join('')}
    ...
    area.querySelectorAll('.bv-card').forEach(card => {
      card.addEventListener('click', e => {
        ...
        area.querySelectorAll('.bv-card.is-selected').forEach(c => c.classList.remove('is-selected'));
        card.classList.add('is-selected');
...
var VocabPage = (() => {
  ...
  let activeLevel    = 'N5';
  let activeChapter  = 0;
  let activeBookId   = null;   // Part 1: selected book within the active level
  let cardIndex      = 0;
  let flipped        = false;
  let speakingId     = null;
  let speakTimer     = null;
  ...
  let step             = 'levels';   // 'levels' | 'chapters' | 'n2weeks' | 'list' | 'detail'
  let chapterSearch     = '';
  let detailWordIdx    = 0;

  /* ── N2-only step-flow state (Main Page → Week Page → Lesson) ──────
     N2 vocabulary follows the official Nihongo Sou Matome N2 book
     structure: 8 weeks, each with lessons 1.1–1.7 (1.7 = practice
     test). `step === 'chapters'` becomes the 8-week Main Page for N2,
     and `step === 'n2weeks'` is the Week Page listing 1.1–1.7. ── */
  let activeN2Week      = null;
  ...
  function getCurrentChapterData() {
    const chapters = getLevelChapters();
  ...
  const LEVEL_COLORS = { N5:'var(--n5)', N4:'var(--n4)', N3:'var(--n3)', N2:'var(--n2)', N1:'var(--n1)' };

  /* ── Theme-consistent level icons (shared with Pronunciation page) ── */
  const LEVEL_ICONS = { N5:'🌱', N4:'🌿', N3:'🌳', N2:'🏯', N1:'🗻' };

  /* ── Japanese (kanji) chapter numbering ──────────────────
     Converts 1-99 → 一,二,…,十,十一,…,二十,… so chapter numbers
     display the way a Japanese textbook actually numbers them. */
  function toKanjiNumeral(n) {
    ...
    return String(n); // beyond textbook chapter range — show as-is
  }

  /* ── Japanese chapter titles ──────────────────────────────
     Prefers real data (ch.titleJa / ch.jpTitle) if the loaded
     chapters.js already provides it. Falls back to this table
     for chapters confirmed against the source textbook; add
     more entries here as they're verified — anything not listed
     falls back to the English title rather than guessing. */
  const CHAPTER_TITLES_JA = {
    'N5-1': 'はじめまして',
    'N5-2': 'これはなんですか',
    'N5-3': 'ここはどこですか',
  };
  function getChapterTitleJa(level, ch) {
    return ch.titleJa || ch.jpTitle || CHAPTER_TITLES_JA[`${level}-${ch.ch}`] || null;
  }

  /* ── Auto-generated example sentences ─────────────────────
     The current word data only has { jp, romaji, en } — no
     example-sentence field. Rather than leaving the sentence
     box empty, we generate a simple, grammatically-safe sentence
     around each word and clearly label it as generated, so it's
     never mistaken for verified textbook content. Cached so the
     same word always shows the same sentence in one session. */
  const exampleCache = new Map();
  const SENTENCE_FRAMES = [
    w => ({
      jp: `これは${w.jp}です。`,
      romaji: `Kore wa ${w.romaji} desu.`,
      en: `This is ${w.en}.`,
    }),
    w => ({
      jp: `${w.jp}はとてもいいです。`,
      romaji: `${w.romaji} wa totemo ii desu.`,
      en: `${w.en} is very good.`,
    }),
    w => ({
      jp: `わたしは${w.jp}がすきです。`,
      romaji: `Watashi wa ${w.romaji} ga suki desu.`,
      en: `I like ${w.en}.`,
    }),
    w => ({
      jp: `あれは${w.jp}じゃありません。`,
      romaji: `Are wa ${w.romaji} ja arimasen.`,
      en: `That is not ${w.en}.`,
    }),
  ];
  function generateExample(word) {
    const key = word.jp + '|' + word.en;
    if (exampleCache.has(key)) return exampleCache.get(key);
    let h = 0;
    for (let i = 0; i < key.length; i++) h = (h * 31 + key.charCodeAt(i)) | 0;
    const frame = SENTENCE_FRAMES[Math.abs(h) % SENTENCE_FRAMES.length];
    const sentence = frame(word);
    exampleCache.set(key, sentence);
    return sentence;
  }

  /* ── Play All / Stop (sequential audio through the current
     chapter's — or filtered — word list) ─────────────────── */
  ...
  function playAllStep(list) {
    if (!playAllActive || playAllIdx >= list.length) { stopPlayAll(); return; }
    const word = list[playAllIdx];
    highlightPlayingCard(word.jp);
    window.speechSynthesis.cancel();
    const u = new SpeechSynthesisUtterance(word.jp);
    u.lang = 'ja-JP'; u.rate = 0.85;
    u.onend = () => {
      if (!playAllActive) return;
      playAllIdx++;
      setTimeout(() => playAllStep(list), 350);
    };
    u.onerror = () => { if (playAllActive) { playAllIdx++; playAllStep(list); } };
    window.speechSynthesis.speak(u);
  }
  ...
  function highlightPlayingCard(jp) {
    document.querySelectorAll('.nz-vcard').forEach(card => {
      card.classList.toggle('nz-vcard-playing', jp !== null && card.dataset.jp === jp);
    });
  }
  function updatePlayAllButtons() {
    const playBtn = document.getElementById('vocab-playall-btn');
    const stopBtn = document.getElementById('vocab-stopall-btn');
    if (playBtn) playBtn.disabled = playAllActive;
    if (stopBtn) stopBtn.disabled = !playAllActive;
  }
  ...
  function renderLevelTabs() {
    ...
    wrap.querySelectorAll('.nz-lvl-tab').forEach(btn=>{
      btn.addEventListener('click',()=>{
        ...
        activeBookId=null; ensureActiveBook();
  ...
  /* ── PART 1: Dynamic Book Selector ────────────────────────────
     Strict level→book rules. N5/N4 have exactly ONE book each (no
     tab bar shown at all). N3/N2/N1 each have exactly TWO books.
     Data structure is future-proof: chapters are matched to a book
     via `chapter.bookId`, so new books can be added later just by
     tagging chapters.js entries — no code here needs to change.  */
  const BOOKS = {
    N5: [ { id:'mnn1',       label:'Minna no Nihongo 1' } ],
    N4: [ { id:'mnn2',       label:'Minna no Nihongo 2' } ],
    N3: [
      { id:'chukyu1',        label:'Chukyu 1' },
      { id:'chukyu2',        label:'Chukyu 2' },
    ],
    N2: [
      { id:'soumatome-n2',   label:'Nihongo Soumatome N2' },
      { id:'shinkanzen-n2',  label:'Shin Kanzen Master N2' },
    ],
    N1: [
      { id:'soumatome-n1',   label:'Nihongo Soumatome N1' },
      { id:'shinkanzen-n1',  label:'Shin Kanzen Master N1' },
    ],
  };
  function getBooksForLevel(level) { return BOOKS[level] || []; }
  function levelHasMultipleBooks(level) { return getBooksForLevel(level).length > 1; }

  /* Ensures activeBookId always points at a valid book for activeLevel. */
  function ensureActiveBook() {
    const books = getBooksForLevel(activeLevel);
    if (!books.length) { activeBookId = null; return; }
    if (!books.some(b => b.id === activeBookId)) activeBookId = books[0].id;
  }

  /* Chapters belonging to the active level, filtered to the active book.
     Preferred match: chapter.bookId === book.id (future-proof — this is
     the only thing new books/chapters need to set).
     Fallback (only if none of the level's chapters carry a bookId yet):
     split the raw chapter array evenly across the level's books so the
     UI still works correctly before chapters.js is tagged. */
  function getLevelChapters() {
    const all = (typeof NZChapterData !== 'undefined' && NZChapterData[activeLevel]) || [];
    /* N2 has two books — Nihongo Soumatome N2 (data already loaded) and
       Shin Kanzen Master N2 (button kept working, data to be added later).
       None of the N2 chapters carry a bookId yet, and every chapter
       currently in NZChapterData.N2 genuinely IS Sou Matome data — so
       (unlike the generic 50/50 fallback below) all of it belongs to
       the Soumatome tab, and Shin Kanzen simply has nothing yet until
       its chapters are tagged with bookId:'shinkanzen-n2'. */
    if (activeLevel === 'N2') {
      const books = getBooksForLevel('N2');
      ensureActiveBook();
      const activeBook = books.find(b => b.id === activeBookId) || books[0];
      const tagged = all.filter(c => c.bookId);
      if (tagged.length) return all.filter(c => c.bookId === activeBook.id);
      return activeBook.id === 'soumatome-n2' ? all : [];
    }
    const books = getBooksForLevel(activeLevel);
    if (books.length <= 1) return all;
    ensureActiveBook();
    const activeBook = books.find(b => b.id === activeBookId) || books[0];
    const tagged = all.filter(c => c.bookId);
    if (tagged.length) return all.filter(c => c.bookId === activeBook.id);
    const bookIdx = books.indexOf(activeBook);
    const per = Math.ceil(all.length / books.length) || 0;
    return all.slice(bookIdx * per, (bookIdx + 1) * per);
  }
  function getActiveBookLabel() {
    const books = getBooksForLevel(activeLevel);
    ensureActiveBook();
    const found = books.find(b => b.id === activeBookId);
    return found ? found.label : (activeLevel || '');
  }

  /* ── Book Selection Bar (Tab Switcher) ─────────────────────────
     Rendered only when the active level has more than one book
     (N3, N2, N1). N5/N4 never render this — single book only. */
  function renderBookTabs(container) {
    if (!levelHasMultipleBooks(activeLevel)) { container.innerHTML = ''; return; }
    const books = getBooksForLevel(activeLevel);
    ensureActiveBook();
    const color = LEVEL_COLORS[activeLevel] || 'var(--primary)';
    container.innerHTML = `
      <div class="nz-book-tabs" role="tablist" aria-label="Select book">
        ${books.map(b => `
          <button class="nz-book-tab ${b.id===activeBookId?'active':''}" data-book="${b.id}" role="tab"
            aria-selected="${b.id===activeBookId}"
            style="--nz-book-color:${color};">
            ${esc(b.label)}
          </button>`).join('')}
      </div>`;
    container.querySelectorAll('.nz-book-tab').forEach(btn => {
      btn.addEventListener('click', () => {
        if (btn.dataset.book === activeBookId) return;
        activeBookId = btn.dataset.book;
        activeChapter = 0;
        chapterSearch = '';
        renderChapterCardsScreen(document.getElementById('vocab-step-area'));
      });
    });
  }
  ...
  function renderChapterBtns() {
    ...
    const chapters = getLevelChapters();
    const color = LEVEL_COLORS[activeLevel] || 'var(--primary)';
    const bookLabel = getActiveBookLabel();
    if (!chapters.length) {
  ...
  /* ── Grid (reference parchment card, one word per card) ── */
  /* Part 2 — Chapter Vocabulary List card. Strict vertical order:
     1) Japanese  2) English meaning  3) Romaji.
     Example sentences / translations are intentionally NOT rendered
     here — this page is for vocabulary browsing only; the full
     sentence lives on the Detail page (renderDetailScreen). */
  function renderVCard(word, i) {
    const num = String(i + 1).padStart(2, '0');
    const hasKana = !!word.reading; // only shown if real reading data exists
    return `
    <div class="nz-vcard" data-jp="${esc(word.jp)}">
      <div class="nz-vcard-head">
        <button class="nz-vcard-speak" data-jp="${esc(word.jp)}" aria-label="Play ${esc(word.jp)}">
          ${speakerIcon(16)}
        </button>
        <div class="nz-vcard-body">
          <div>
            <span class="nz-vcard-num">${num}</span>
            <span class="nz-vcard-jp">${esc(word.jp)}</span>
            ${hasKana ? `<span class="nz-vcard-kana">${esc(word.reading)}</span>` : ''}
          </div>
          <div class="nz-vcard-en">${esc(word.en)}</div>
          <div class="nz-vcard-romaji">${esc(word.romaji || '')}</div>
        </div>
      </div>
    </div>`;
  }
  ...
  function renderGrid() {
    ...
    area.innerHTML=`
<div class="nz-vcard-page">
  <div class="nz-vcard-list">
    ${filtered.map((word,i)=>renderVCard(word,i)).join('')}
  </div>
</div>`;
    area.querySelectorAll('.nz-vcard').forEach(card=>{
      card.addEventListener('click',e=>{
        if(e.target.closest('.nz-vcard-speak')) return;
        openDetail(card.dataset.jp);
      });
    });
    area.querySelectorAll('.nz-vcard-speak').forEach(btn=>{
      btn.addEventListener('click',e=>{
        e.stopPropagation();
  ...
  function renderFCContent() {
    ...
    if(inner) inner.classList.toggle('flipped',flipped);
    const col = LEVEL_COLORS[activeLevel] || 'var(--n5)';
    if(front) front.innerHTML=`
...
<div style="display:flex;flex-direction:column;align-items:center;max-width:380px;margin:0 auto;">
  ...
  function renderLevelCardsScreen(area) {
    const lbl = document.getElementById('vocab-count-label');
    if (lbl) lbl.textContent = 'Choose a JLPT level to begin your vocabulary journey';

    if (!window.nzLevelDashboard) {
      // Fallback if the shared component hasn't loaded yet.
      area.innerHTML = `<div class="nz-level-grid">${['N5','N4','N3','N2','N1'].map(lvl => {
        const col = LEVEL_COLORS[lvl], stats = levelStats(lvl);
        return `<button class="nz-level-card" data-level="${lvl}" style="border-color:${col}33;">
      ...
      area.querySelectorAll('.nz-level-card').forEach(btn => {
        btn.addEventListener('click', () => { window.nzVocabGoLevel(btn.dataset.level); });
      });
      return;
    }

    const savedFav = (window.getSavedItems && window.getSavedItems().vocab) || {};
    let totalWordsAll = 0, learnedAll = 0;

    const levelCfgs = ['N5','N4','N3','N2','N1'].map(lvl => {
      const col = LEVEL_COLORS[lvl];
      const stats = levelStats(lvl);
      const prog = window.nzProgGet ? window.nzProgGet('vocab', lvl) : {};
      const completedCount = (prog.completed || []).length;
      const favCount = Object.keys(savedFav).filter(id => savedFav[id].level === lvl).length;
      const pct = stats.wordCount ? Math.min(100, Math.round(completedCount / stats.wordCount * 100)) : 0;
      totalWordsAll += stats.wordCount; learnedAll += completedCount;
      return {
        level: lvl, color: col, icon: LEVEL_ICONS[lvl],
        metaLines: [stats.chapterCount + ' Chapters', stats.wordCount + ' Words'],
        pct,
        innerStats: [
          { icon: '📅', label: 'Last Studied', value: window.nzRelativeTime ? window.nzRelativeTime(prog.lastStudied) : 'Not Studied Yet' },
          { icon: '🔖', label: 'Favorites', value: favCount }
        ],
        buttonLabel: prog.lastStudied ? 'Continue' : 'Start Learning',
        onClick: `window.nzVocabGoLevel('${lvl}')`
      };
    });

    const streak = (window._nzUserData && window._nzUserData.streak) || 0;
    const overallPct = totalWordsAll ? Math.round(learnedAll / totalWordsAll * 100) : 0;

    area.innerHTML = window.nzLevelDashboard({
      topStats: [
        { icon: '📖', value: totalWordsAll, label: 'Total Words', color: 'var(--n2)' },
        { icon: '✅', value: learnedAll, label: 'Words Learned', color: 'var(--success, #22c55e)' },
        { icon: '🔥', value: streak, label: 'Day Streak', color: 'var(--accent)' }
      ],
      ringPct: overallPct, ringColor: 'var(--primary)', ringLabel: 'Overall Progress',
      levels: levelCfgs,
      fox: { message: 'New words every day,', sub: 'better Japanese every day! ✨', img: 'assets/fox/fox-wave.png' },
      goal: { icon: '🎯', title: 'Daily Goal', text: 'Study 20 new words today to keep your streak going!', ctaLabel: 'Start Today', ctaColor: 'var(--n5)', onClick: `window.nzVocabGoLevel('N5')` }
    });
  }

  window.nzVocabGoLevel = (lvl) => {
    activeLevel = lvl; activeChapter = 0; chapterSearch = ''; activeBookId = null;
    ensureActiveBook();
    if (window.nzMarkStudied) window.nzMarkStudied('vocab', lvl);
    goToChapters();
  };

  /* ═══════════════════════════════════════════════════════════
     N2 — RESTRUCTURED NAVIGATION (Sou Matome N2 book structure)
     Main Page (8 Week cards) → Week Page (1.1–1.7) → Lesson.
     Lesson 1.7 of every week reuses the existing, already-built
     N2 Practice Test page instead of opening a vocabulary list.
     ═══════════════════════════════════════════════════════════ */

  /* All N2 chapters for the currently active book (Soumatome or Shin
     Kanzen) — this is the single source of truth the Main Page, Week
     Page and the existing List/Detail screens all index into via
     activeChapter, so it must match getLevelChapters() exactly. */
  function n2AllChapters() {
    return getLevelChapters();
  }
  function n2ChaptersForWeek(week) {
    return n2AllChapters()
      .filter(c => c.week === week)
      .sort((a, b) => a.day - b.day);
  }
  /* The book doesn't carry a distinct "week name" field — each week's
     day-1 lesson title is what actually identifies the week (e.g. Week 1's
     day-1 lesson is "Apartment hunting"), so that's used as the card title. */
  function n2WeekTitle(week) {
    const chs = n2ChaptersForWeek(week);
    const first = chs.find(c => c.day === 1) || chs[0];
    return (first && first.title) || `Week ${week}`;
  }

  /* ── N2 STEP 1: Main Page — 8 Week cards only, no CH/Chapter labels ──
     Book selector bar restored (Nihongo Soumatome N2 / Shin Kanzen
     Master N2) exactly like it worked before — switching book tabs
     just re-renders this same Main Page against the selected book's
     chapters. Shin Kanzen has no data loaded yet, so its tab works
     and stays clickable, showing a "coming soon" state until its
     chapters are added and tagged with bookId:'shinkanzen-n2'. */
  function renderN2MainScreen(area) {
    ensureActiveBook();
    const bookLabel = getActiveBookLabel();
    const all = n2AllChapters();
    const lbl = document.getElementById('vocab-count-label');
    if (lbl) lbl.textContent = `${bookLabel} · 8 weeks`;

    const weeks = Array.from(new Set(all.map(c => c.week))).sort((a, b) => a - b);

    area.innerHTML = `
      ${crumbsHtml([{ label: '語彙 Vocabulary' }, { label: 'N2' }])}
      <div id="vocab-book-tabs"></div>
      ${!weeks.length
        ? `<div class="card" style="padding:40px;text-align:center;color:var(--fg-muted);">
            <p style="font-size:13px;margin:0;">${esc(bookLabel)} data is coming soon.</p>
          </div>`
        : `<div class="nz-chapter-grid">
            ${weeks.map(week => {
              const chs = n2ChaptersForWeek(week);
              const wordCount = chs.reduce((sum, c) => sum + ((c.words || []).length), 0);
              const prog = window.nzProgGet ? window.nzProgGet('vocab', 'N2-W' + week) : {};
              const completed = (prog && prog.completed) ? true : null;
              return `
              <button class="nz-chapter-card" data-week="${week}" style="--jlpt-c:${LEVEL_COLORS.N2}">
                <div class="nz-ch-title">${esc(n2WeekTitle(week))}</div>
                <div class="nz-ch-topic">Week ${week}</div>
                <div class="nz-ch-footer">
                  <span>${wordCount} words</span>
                  ${completed ? `<span class="nz-ch-complete-badge">✓ Completed</span>` : ''}
                </div>
              </button>`;
            }).join('')}
          </div>`}
    ...
    renderBookTabs(document.getElementById('vocab-book-tabs'));
    area.querySelectorAll('.nz-chapter-card').forEach(card => {
      card.addEventListener('click', () => {
        activeN2Week = parseInt(card.dataset.week);
        goToN2Weeks();
      });
    });
  }

  /* ── N2 STEP 2: Week Page — lessons 1.1 through 1.7 ──
     1.1–1.6 open the existing vocabulary lesson (List screen).
     1.7 opens the existing, already-implemented Practice Test page. */
  function renderN2WeekScreen(area) {
    const chs = n2ChaptersForWeek(activeN2Week);

    area.innerHTML = `
      <button class="nz-flow-back" id="vocab-back-n2main">${backArrowSvg()} Back to N2 Vocabulary</button>
      ${crumbsHtml([{ label: '語彙 Vocabulary' }, { label: 'N2' }, { label: n2WeekTitle(activeN2Week) }])}
      <div class="nz-chapter-grid">
        ${chs.map(ch => {
          const isTest = !!ch.isTest;
          return `
          <button class="nz-chapter-card" data-label="${esc(ch.label)}" style="--jlpt-c:${LEVEL_COLORS.N2}">
            <span class="nz-ch-num" style="background:${LEVEL_COLORS.N2}1e;color:${LEVEL_COLORS.N2};">${esc(ch.label)}</span>
            <div class="nz-ch-title">${isTest ? '📝 Practice Test' : esc(ch.title)}</div>
            <div class="nz-ch-topic">${esc(ch.topic || '')}</div>
            <div class="nz-ch-footer">
              <span>${isTest ? (ch.quiz ? ch.quiz.length : 0) + ' questions' : (ch.words || []).length + ' words'}</span>
            </div>
          </button>`;
        }).join('')}
      </div>
    ...
    document.getElementById('vocab-back-n2main')?.addEventListener('click', goToChapters);
    area.querySelectorAll('.nz-chapter-card').forEach(card => {
      card.addEventListener('click', () => {
        const ch = chs.find(c => c.label === card.dataset.label);
        if (!ch) return;
        if (ch.isTest) {
          window._nzPendingN2Week = ch.week;
          if (window.nzGo) window.nzGo('n2-practice-tests');
          return;
        }
        activeChapter = n2AllChapters().indexOf(ch);
        goToList();
      });
    });
  }

  function goToN2Weeks() { stopPlayAll(); step = 'n2weeks'; renderStepArea(); }

  /* ── STEP 2: Chapter Cards (this is the "JLPT Level page") ──
     Layout order per spec: Main Heading → Book Selector Bar →
     Chapter Search Bar → Chapters grid. N2 is restructured to its
     own Week-based Main Page — see renderN2MainScreen() above. */
  function renderChapterCardsScreen(area) {
    if (activeLevel === 'N2') { renderN2MainScreen(area); return; }
    const col = LEVEL_COLORS[activeLevel] || 'var(--primary)';
    ensureActiveBook();
    const bookLabel = getActiveBookLabel();
    const allChapters = getLevelChapters();
    const lbl = document.getElementById('vocab-count-label');
      ...
      ${!chapters.length
        ...
        : `<div class="nz-chapter-grid">
            ${chapters.map(ch => {
              ...
              const titleJa = getChapterTitleJa(activeLevel, ch);
              return `
              <button class="nz-chapter-card" data-idx="${idx}" style="--jlpt-c:${col}">
                <span class="nz-ch-num" style="background:${col}1e;color:${col};">${toKanjiNumeral(ch.ch)}</span>
                ${titleJa
                  ? `<div class="nz-ch-title">${esc(titleJa)}</div>
                     <div class="nz-ch-topic">${esc(ch.title)}${ch.topic ? ' · ' + esc(ch.topic) : ''}</div>`
  ...
  function renderListScreen(area) {
    ...
    area.innerHTML = `
      <button class="nz-flow-back" id="vocab-back-chapters">${backArrowSvg()} ${isN2 ? 'Back to Week' : 'Back to Chapters'}</button>
      ${isN2
        ? crumbsHtml([{ label:'語彙 Vocabulary' }, { label: 'N2' }, { label: n2WeekTitle(activeN2Week) }, { label: ch.label || `Ch ${ch.ch}` }])
      ...
      <div class="nz-playall-bar">
        <button id="vocab-playall-btn" class="nz-playall-btn">▶ Play All</button>
        <button id="vocab-stopall-btn" class="nz-stopall-btn" disabled>■ Stop</button>
      </div>
      <div id="vocab-main-area"></div>
    `;
    document.getElementById('vocab-back-chapters')?.addEventListener('click', () => { stopPlayAll(); isN2 ? goToN2Weeks() : goToChapters(); });
    document.getElementById('vocab-search-input')?.addEventListener('input', e => {
      stopPlayAll();
      searchQuery = e.target.value; cardIndex = 0;
      if (mode === 'grid') renderGrid(); else renderFCContent();
    });
    document.getElementById('vocab-playall-btn')?.addEventListener('click', playAllWords);
    document.getElementById('vocab-stopall-btn')?.addEventListener('click', stopPlayAll);
    renderModeBtns();
    updateChapterInfo();
  ...
  /* Part 3 audio: Vocabulary → short pause → Example sentence. */
  function speakWordThenSentence(word, ex) {
    if (!window.speechSynthesis) return;
    window.speechSynthesis.cancel();
    const w = new SpeechSynthesisUtterance(word.jp);
    w.lang = 'ja-JP'; w.rate = 0.8;
    w.onend = () => setTimeout(() => {
      const s = new SpeechSynthesisUtterance(ex.jp);
      s.lang = 'ja-JP'; s.rate = 0.8;
      window.speechSynthesis.speak(s);
    }, 700); // short pause between vocabulary and sentence
    window.speechSynthesis.speak(w);
  }

  /* Part 3 — Vocabulary Detail page. Strict vertical order:
     1) Japanese  2) English meaning  3) Romaji  4) Example sentence
     5) Sentence meaning. Example sentence is shown ONLY here. */
  function renderDetailScreen(area) {
    ...
    const hasKana = !!word.reading;

    area.innerHTML = `
      <button class="nz-flow-back" id="vocab-back-list">${backArrowSvg()} Back to List</button>
      ${crumbsHtml([{ label:'語彙 Vocabulary' }, { label: activeLevel }, { label:`${toKanjiNumeral(ch.ch)} ${ch.title}` }, { label: word.jp }])}
      <div class="nz-vcard-page nz-vcard-detail-wrap">
        <div class="nz-vcard" data-jp="${esc(word.jp)}">
          <div class="nz-vcard-head">
            <button class="nz-vcard-speak" id="vocab-detail-speak" aria-label="Play ${esc(word.jp)} and example sentence">
              ${speakerIcon(18)}
            </button>
            <div class="nz-vcard-body">
              <div>
                <span class="nz-vcard-num">${String(detailWordIdx+1).padStart(2,'0')}</span>
                <span class="nz-vcard-jp">${esc(word.jp)}</span>
                ${hasKana ? `<span class="nz-vcard-kana">${esc(word.reading)}</span>` : ''}
              </div>
              <div class="nz-vcard-en">${esc(word.en)}</div>
          ...
          <div class="nz-vcard-sentence">
            <div class="nz-vcard-sentence-jp">${esc(ex.jp)}</div>
            <div class="nz-vcard-sentence-romaji">${esc(ex.romaji)}</div>
            <div class="nz-vcard-sentence-en">${esc(ex.en)}</div>
            <div class="nz-vcard-generated-tag">AI-generated example</div>
          </div>
          <div class="nz-vcard-detail-nav">
    ...
    document.getElementById('vocab-detail-speak')?.addEventListener('click', () => speakWordThenSentence(word, ex));
  ...
  function renderStepArea() {
    ...
    switch (step) {
      ...
      case 'n2weeks':  renderN2WeekScreen(area); break;
      case 'list':     renderListScreen(area); break;
      case 'detail':   renderDetailScreen(area); break;
    }
  }

  function goToLevels()   { stopPlayAll(); step = 'levels';   renderStepArea(); }
  function goToChapters() { stopPlayAll(); step = 'chapters'; renderStepArea(); }
  ...
  function mount(containerId, initialLevel) {
    ...
    detachKeys();
    mode = 'grid'; cardIndex = 0; flipped = false; searchQuery = '';
    step = 'levels'; chapterSearch = ''; detailWordIdx = 0; playAllActive = false;
    activeBookId = null; activeN2Week = null;
...
<div style="max-width:1280px;margin:0 auto;padding:24px 16px;">
  ...
  <div style="display:flex;align-items:flex-start;justify-content:space-between;
    ...
    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
      <!-- Basic Vocab button — only shown for N5, where the Basic
           Vocab word bank (2,156 words / 20 categories) applies -->
      ${activeLevel === 'N5' ? `<button id="bv-open-btn">📚 Basic Vocab</button>` : ''}
    </div>
  </div>
  ...
  <div id="vocab-step-area"></div>
    ...
    /* Basic Vocab button (only present when activeLevel === 'N5') */
    const bvOpenBtn = document.getElementById('bv-open-btn');
    if (bvOpenBtn) bvOpenBtn.addEventListener('click',()=>{ if (window.nzGo) window.nzGo('basic-vocab'); });
...
var GrammarPage = (() => {
  ...
  function renderLevelCardsScreen(area) {
    ...
    if (lbl) lbl.textContent = 'Choose a JLPT level to begin your grammar journey';
    if (!window.nzLevelDashboard) {
      // Fallback if the shared component hasn't loaded yet.
      area.innerHTML = `<div class="nz-level-grid">${['N5','N4','N3','N2','N1'].map(lvl => {
        const col = LEVEL_COLORS[lvl], stats = levelStats(lvl);
      ...
      area.querySelectorAll('.nz-level-card').forEach(btn => {
        btn.addEventListener('click', () => { activeLevel = btn.dataset.level; activeChapter = 0; chapterSearch = ''; goToChapters(); });
      });
      return;
    }

    const savedFav = (window.getSavedItems && window.getSavedItems().grammar) || {};
    let totalPointsAll = 0, learnedAll = 0;

    const levelCfgs = ['N5','N4','N3','N2','N1'].map(lvl => {
      const col = LEVEL_COLORS[lvl];
      const stats = levelStats(lvl);
      const prog = window.nzProgGet ? window.nzProgGet('grammar', lvl) : {};
      const completedCount = (prog.completed || []).length;
      const favCount = Object.keys(savedFav).filter(id => savedFav[id].level === lvl).length;
      const pct = stats.pointCount ? Math.min(100, Math.round(completedCount / stats.pointCount * 100)) : 0;
      totalPointsAll += stats.pointCount; learnedAll += completedCount;
      return {
        level: lvl, color: col, icon: LEVEL_ICONS[lvl],
        metaLines: [stats.chapterCount + ' Chapters', stats.pointCount + ' Grammar Points'],
        pct,
        innerStats: [
          { icon: '📅', label: 'Last Studied', value: window.nzRelativeTime ? window.nzRelativeTime(prog.lastStudied) : '—' },
          { icon: '🔖', label: 'Favorites', value: favCount }
        ],
        buttonLabel: prog.lastStudied ? 'Continue Learning' : 'Start Learning',
        onClick: `window.nzGrammarGoLevel('${lvl}')`
      };
    });

    const streak = (window._nzUserData && window._nzUserData.streak) || 0;
    const overallPct = totalPointsAll ? Math.round(learnedAll / totalPointsAll * 100) : 0;

    area.innerHTML = window.nzLevelDashboard({
      topStats: [
        { icon: '📘', value: totalPointsAll, label: 'Total Grammar Points', color: 'var(--n3)' },
        { icon: '✅', value: learnedAll, label: 'Learned Points', color: 'var(--success, #22c55e)' },
        { icon: '🔥', value: streak, label: 'Day Streak', color: 'var(--accent)' }
      ],
      ringPct: overallPct, ringColor: 'var(--primary)', ringLabel: 'Overall Progress',
      levels: levelCfgs,
      fox: { message: 'Grammar is the structure that makes your Japanese strong and natural.', sub: 'Keep practicing! ✨', img: 'assets/fox/fox-blackboard-teach.png' },
      goal: { icon: '💡', title: 'Tip', text: 'Learn a little grammar every day and try using it in sentences. Practice makes perfect!', ctaLabel: 'Daily Practice', ctaColor: 'var(--n5)', onClick: `window.nzGrammarGoLevel('N5')` }
    });

    window.nzGrammarGoLevel = (lvl) => {
      activeLevel = lvl; activeChapter = 0; chapterSearch = '';
      if (window.nzMarkStudied) window.nzMarkStudied('grammar', lvl);
      goToChapters();
    };
  }
  ...
  function renderListScreen(area) {
      ...
      ${!pts.length
        ...
        : `<div style="display:flex;flex-direction:column;gap:10px;">
            ${pts.map((p) => {
              ...
              const favId = `${activeLevel}-ch${ch.ch}-${realIdx}`;
              const isFav = window.isSaved && window.isSaved('grammar', favId);
              return `
              <div class="nz-gpoint-card">
                <div class="nz-gpoint-header" data-idx="${realIdx}">
                  ...
                  <div style="display:flex;align-items:center;gap:8px;">
                    <button class="nz-gpoint-fav-btn" data-fav-id="${favId}" data-fav-level="${activeLevel}"
                      style="background:none;border:none;cursor:pointer;font-size:16px;padding:2px;color:${col};" title="Favorite">
                      ${isFav ? '🔖' : '📑'}</button>
    ...
    area.querySelectorAll('.nz-gpoint-fav-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        if (window.nzToggleSaved) window.nzToggleSaved('grammar', btn.dataset.favId, { level: btn.dataset.favLevel });
        renderListScreen(area);
      });
    });
  }
  ...
  function openDetail(pointIdx) {
    detailIdx = pointIdx;
    step = 'detail';
    if (window.nzMarkCompleted) {
      const ch = currentChapter();
      if (ch) window.nzMarkCompleted('grammar', activeLevel, `${activeLevel}-ch${ch.ch}-${pointIdx}`);
    }
    renderStepArea();
  }
...
<div style="max-width:1280px;margin:0 auto;padding:24px 16px;">
  <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:20px;">
    ...
    <button onclick="nzGo('verb-conjugation')" style="display:flex;align-items:center;gap:6px;padding:9px 16px;border-radius:10px;font-size:13px;font-weight:700;font-family:inherit;cursor:pointer;white-space:nowrap;border:1.5px solid var(--primary);background:var(--primary-dim);color:var(--primary);">
...
(function injectKanjiStyles() {
  ...
  style.textContent = `
    ...
    .knj-srs-N4 { color:var(--n4); border-color:var(--n4) !important; }
    .knj-srs-N3 { color:var(--n3); border-color:var(--n3) !important; }
    .knj-srs-N2 { color:var(--n2); border-color:var(--n2) !important; }
    .knj-srs-N1 { color:var(--n1); border-color:var(--n1) !important; }
...
/* Chapter button removed per UI/UX update — see note near
   NZChapterVocab above. NZChapterController.onModalOpen() (which wires
   the reading/listening/quiz tab bar into the chapter overlay) is left
   intact and callable, in case another entry point opens that overlay
   in the future; only the auto-injected "📖 Chapters" button itself
   (id="nzcc-open-btn") is disabled. No NZChapterData is touched. */
