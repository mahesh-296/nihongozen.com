...
var VocabPage = (() => {
  ...
  /* ── Level icons — reuses the single shared window.jlptIcon() map
     (defined once in index.html) so Vocabulary always matches Grammar,
     Listening, Reading, Kanji, etc. Falls back locally only if that
     shared function isn't loaded yet for some reason. ─────────────── */
  function lvlIcon(lvl) {
    if (typeof window.jlptIcon === 'function') return window.jlptIcon(lvl);
    return { N5:'🌱', N4:'🌿', N3:'🌳', N2:'🏯', N1:'🗻' }[lvl] || '📘';
  }
  const LEVEL_ICONS = new Proxy({}, { get: (_, lvl) => lvlIcon(lvl) });
  ...
  function renderChapterBtns() {
    ...
    c.innerHTML = `
      <div style="display:flex;flex-direction:column;gap:10px;width:100%;">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;">
          <span style="display:inline-flex;align-items:center;gap:7px;padding:6px 14px;
                       border-radius:10px;font-size:12.5px;font-weight:800;letter-spacing:0.01em;
                       background:${color}1f;color:${color};border:1px solid ${color}55;white-space:nowrap;">
            <span style="font-size:14px;">📖</span>${esc(bookLabel)}
          </span>
          <button id="vocab-playall-btn"
            style="display:inline-flex;align-items:center;gap:6px;padding:6px 14px;border-radius:10px;
                   font-size:12.5px;font-weight:700;font-family:inherit;cursor:pointer;transition:all 0.15s;
                   border:1px solid ${color};background:transparent;color:${color};">
            ${speakerIcon(13)} <span id="vocab-playall-label">Play All</span>
          </button>
        </div>
    ...
    const pab = document.getElementById('vocab-playall-btn');
    if (pab) pab.addEventListener('click', togglePlayAll);
  }

  /* ── "Play All" — sequential playback of every word in the
     currently displayed (filtered) chapter list ───────────── */
  ...
  function togglePlayAll() {
    ...
    playAllIndex   = 0;
    updatePlayAllBtn();
    playNextInSequence(words);
  }

  function playNextInSequence(words) {
    if (playAllStopped || playAllIndex >= words.length) { stopPlayAll(); return; }
    const word = words[playAllIndex];
    speakingId = word.jp;
    updateSpeakBtns();
    if (!window.speechSynthesis) { stopPlayAll(); return; }
    window.speechSynthesis.cancel();
    const u = new SpeechSynthesisUtterance(word.jp);
    u.lang = 'ja-JP'; u.rate = 0.85;
    u.onend = () => {
      if (playAllStopped) return;
      playAllIndex++;
      setTimeout(() => playNextInSequence(words), 350);
    };
    u.onerror = () => { if (!playAllStopped) { playAllIndex++; setTimeout(() => playNextInSequence(words), 350); } };
    window.speechSynthesis.speak(u);
  }
  ...
  function updatePlayAllBtn() {
    const btn = document.getElementById('vocab-playall-btn');
    const lbl = document.getElementById('vocab-playall-label');
    if (!btn || !lbl) return;
    lbl.textContent = playAllActive ? 'Stop' : 'Play All';
    btn.style.background = playAllActive ? (LEVEL_COLORS[activeLevel] || 'var(--primary)') : 'transparent';
    btn.style.color = playAllActive ? '#fff' : (LEVEL_COLORS[activeLevel] || 'var(--primary)');
  }
...
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px;">
  ...
  /* ── STEP 2: Detailed Vocabulary Card ─────────────────────
     Opens full-page over the comprehensive list when a word is
     tapped. Preserves the existing detail-card feel (big word,
     furigana/romaji, meaning, tap-to-play audio) and gracefully
     shows an example sentence only if that word's data has one. */
  let detailWord = null;
  ...
  function renderDetailView() {
    const area = document.getElementById('vocab-main-area');
    if (!area || !detailWord) return;
    const word  = detailWord;
    const color = LEVEL_COLORS[activeLevel] || '#22c55e';
    const hasExample = !!(word.example || word.exampleJp || word.sentence);
    const exJp = word.exampleJp || word.example || word.sentence || '';
    const exReading = word.exampleReading || word.exampleKana || '';
    const exRomaji  = word.exampleRomaji || '';
    const exEn      = word.exampleEn || word.exampleTranslation || '';

    area.innerHTML = `
      <div style="max-width:560px;margin:0 auto;">
        <button id="vocab-detail-back"
        ...
        <div style="border-radius:20px;border:1px solid ${color}55;background:var(--card);
                    padding:32px 24px;text-align:center;position:relative;overflow:hidden;">
          <span style="position:absolute;top:14px;right:16px;padding:3px 10px;border-radius:6px;
                       font-size:11px;font-weight:800;background:${color}22;color:${color};">
            ${lvlIcon(activeLevel)} ${esc(activeLevel)}
          </span>
          <div style="font-family:'Noto Sans JP',sans-serif;font-size:44px;font-weight:800;
                      color:var(--fg);margin-bottom:8px;">${esc(word.jp)}</div>
          <p style="font-family:'JetBrains Mono',monospace;font-size:14px;color:var(--fg-muted);
                    font-style:italic;margin-bottom:16px;">${esc(word.romaji||'')}</p>
          <p style="font-size:19px;font-weight:700;color:var(--fg);margin-bottom:20px;">${esc(word.en)}</p>
          <button id="vocab-detail-speak"
            style="display:inline-flex;align-items:center;gap:8px;padding:10px 20px;border-radius:12px;
                   border:1px solid ${color};background:${color}18;color:${color};
                   font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;">
            ${speakerIcon(15)} Tap to hear
          </button>
        </div>
        ${hasExample ? `
        <div style="margin-top:16px;border-radius:16px;border:1px solid var(--border);
                    background:var(--card-elevated);padding:18px 20px;">
          <p style="font-size:10px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;
                    color:var(--fg-muted);margin-bottom:10px;">Example</p>
          <p style="font-family:'Noto Sans JP',sans-serif;font-size:17px;color:var(--fg);margin-bottom:4px;">${esc(exJp)}</p>
          ${exReading ? `<p style="font-size:12px;color:var(--fg-muted);margin-bottom:4px;">${esc(exReading)}</p>` : ''}
          ${exRomaji ? `<p style="font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--fg-muted);margin-bottom:6px;">${esc(exRomaji)}</p>` : ''}
          ${exEn ? `<p style="font-size:13px;font-style:italic;color:var(--fg-muted);">${esc(exEn)}</p>` : ''}
          <button id="vocab-detail-example-speak"
            style="margin-top:12px;display:inline-flex;align-items:center;gap:6px;padding:6px 12px;
                   border-radius:8px;border:1px solid ${color}55;background:transparent;color:${color};
                   font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;">
            ${speakerIcon(12)} Play example
          </button>
        </div>` : ''}
      </div>`;

    document.getElementById('vocab-detail-back')?.addEventListener('click', closeDetail);
    document.getElementById('vocab-detail-speak')?.addEventListener('click', () => speakWord(word.jp, word.en));
    document.getElementById('vocab-detail-example-speak')?.addEventListener('click', () => speak(exJp));
  }
...
<div style="max-width:1600px;margin:0 auto;padding:24px 16px;">
  ...
  <!-- ═══ Level tabs (full width row) ═══ -->
  <div id="vocab-level-tabs"
    style="display:flex;align-items:center;gap:4px;background:var(--card-elevated,#1a1a1a);
           border:1px solid var(--border,#2a2a2a);border-radius:12px;padding:4px;
           overflow-x:auto;scrollbar-width:none;margin-bottom:12px;">
  </div>

  <!-- ═══ Search bar (own full-width row, directly under levels) ═══ -->
  <div class="nz-search-wrap" style="width:100%;margin-bottom:16px;">
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--fg-muted)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
    <input id="vocab-search-input" type="text" placeholder="Search vocab, meaning…" style="width:100%;" />
  </div>
