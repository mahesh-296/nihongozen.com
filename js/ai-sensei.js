...
Respond in the user's language. Be practical, specific, and motivating. Keep responses under 300 words unless a full roadmap is requested.`;
  ...
  // How many past exchanges to keep in memory / send to the API.
  // Keeps requests fast and cheap without losing recent context.
  var MAX_HISTORY = 24;

  /* ─────────────────────────────────────────
     PERSISTENT CHAT HISTORY
     Chat survives page reloads, tab switches, and re-opening the
     panel — scoped per signed-in user so accounts never see each
     other's history on a shared device.
  ───────────────────────────────────────── */
  function historyKey() {
    var uid = (window._nzUser && window._nzUser.uid) || 'guest';
    return 'nz_ai_sensei_history_' + uid;
  }
  ...
  // Friendly names for context injection — falls back to the raw
  // hash for any page/section not listed here.
  var PAGE_NAMES = {
    dashboard:    'Dashboard',
    kana:         'Hiragana/Katakana practice',
    kanji:        'Kanji study',
    vocab:        'Vocabulary',
    grammar:      'Grammar',
    ...
    jlpt:         'JLPT practice test',
    verbs:        'Verb groups & conjugation',
    quiz:         'Kanji quiz'
  };

  function currentPageLabel() {
    var raw = (window.location.hash || '#dashboard').replace('#', '').trim() || 'dashboard';
    return PAGE_NAMES[raw] || raw;
  }
...
`;
  ...
  // Re-populate the chat panel with previously saved messages (if any),
  // so returning users see their conversation instead of starting over.
  ...
  async function processVoiceInput(text) {
    ...
    try {
      var reply = await callGemini(chatHistory.slice(-MAX_HISTORY), SYSTEM_PROMPT);
      chatHistory.push({ role: 'assistant', content: reply });
      saveHistory();
      voiceState = 'speaking';
  ...
  async function askCareer(question) {
    ...
    var careerInput = document.getElementById('as-career-input');
