...
(function () {
  ...
  function renderPrompt(q) {
    ...
    if (q.type === 'icon-match') {
      return '<div class="nz-kq-glyph" style="margin-bottom:8px;">' + esc(k.kanji) + '</div>' +
        '<div style="font-size:16px;font-weight:800;color:var(--fg);margin-bottom:10px;">' + esc(k.meaning) + '</div>' +
        '<div style="display:flex;gap:16px;justify-content:center;flex-wrap:wrap;margin-bottom:10px;font-size:12px;color:var(--fg-muted);">' +
          '<span><b style="color:var(--fg);">On:</b> ' + esc(k.on || '—') + '</span>' +
          '<span><b style="color:var(--fg);">Kun:</b> ' + esc(k.kun || '—') + '</span>' +
        '</div>' +
        (k.example ?
          '<div style="font-family:\'Noto Sans JP\',sans-serif;font-size:13px;color:var(--fg-muted);' +
            'border-top:1px solid var(--border);padding-top:8px;margin-top:2px;">' + esc(k.example) + '</div>' +
          '<div style="font-size:11px;color:var(--fg-subtle);margin-top:2px;">' + esc(k.exampleMeaning || '') + '</div>'
        : '') +
        '<div style="font-size:11.5px;color:var(--fg-muted);margin-top:10px;">' + esc(q.prompt) + '</div>';
    }
