...
// Fallback: show page after 5s if Firebase is slow / offline
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (!user) {
    // Guard against redirect loops: only redirect if not already on login.html
  ...
  // Expose globally for other page scripts
  window._nzAuth    = auth;
  window._nzDb      = db;
  window._nzUser    = user;
  window._nzSignOut = () =>
  ...
  // ── Fetch fresh data and expose it ───────────────────────
