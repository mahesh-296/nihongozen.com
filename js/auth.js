...
onAuthStateChanged(auth, async (user) => {
  ...
  try {
    ...
    // FIX-H (revised): Default onboardingComplete to false in this fallback.
    // Defaulting to true was causing brand-new users to be dropped straight
    // onto the dashboard whenever their very first Firestore write hit an
    // error — skipping onboarding entirely. Defaulting to false is the safer
    // failure mode: a genuinely onboarded user who hits a rare hiccup will
    // just see onboarding once more (harmless), instead of new users
    // silently skipping it (broken).
    window._nzUserData = {
      uid:         user.uid,
      displayName: user.displayName || "Learner",
      email:       user.email       || "",
