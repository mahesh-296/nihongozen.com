...
(function () {
  ...
  /* ---------------- bootstrap: watch for the Vocabulary page mounting ----------------
     NOTE: With the redesigned Vocabulary flow (Level Select → Chapter Cards → List →
     Detail), #vocab-main-area only exists while the List step is showing — it is not
     present the instant #nz-vocab-root first appears. So instead of a one-shot init
     guarded by a root-level flag, we re-check every time the DOM changes and (re)wire
     up enhancements whenever a fresh #vocab-main-area / #vocab-mode-btns shows up.
     All the actual enhancement functions above are already idempotent
     (enhanceCards skips cards already marked data-nz-adv, injectFilterButton skips if
     its button already exists), so calling this often is safe. */
  function initAdvanced(root) {
    loadFilters();
    const mainArea = root.querySelector('#vocab-main-area');
    if (mainArea && !mainArea.dataset.nzAdvObs) {
      mainArea.dataset.nzAdvObs = '1';
      loadProgress().then(() => enhanceCards(mainArea));
  ...
  function watchForVocabPage() {
    ...
    if (existing) initAdvanced(existing);
  }
